...
must_haves:
  truths:
    ...
    - "All 8 new test files exist and contain behavior expectations (not tautologies)"
  ...
  artifacts:
    ...
    - path: "tests/initiate-call-fn.test.ts"
      ...
      min_lines: 50
    - path: "tests/agent-setup.test.ts"
      provides: "Failing stub for CALL-02 TCPA preamble in LLM begin_message"
      min_lines: 20
  ...
  key_links:
    ...
    - from: "tests/agent-setup.test.ts"
...
Output: 8 test files (most failing), 2 SQL migrations, updated verify-signatures.ts, src/types/calls.ts.
</objective>
...
<task type="auto" tdd="true">
  ...
  <behavior>
    ...
    - agent-setup.test.ts: when existingAgentId is null, client.llm.create is called with begin_message containing "artificial intelligence" and "recorded" (CALL-02)
  ...
  <action>
Create all 8 test files. Follow the Phase 1 stub pattern: modules imported dynamically so tests fail with "cannot find module" until implementation exists — this is the intentional red state.
...
**tests/agent-setup.test.ts** — covers CALL-02 TCPA preamble enforcement in the LLM begin_message. Will fail "Cannot find module" until plan 02-02 creates setup-retell-agent.ts:
...
agent-setup.test.ts: fails with "Cannot find module ../netlify/functions/setup-retell-agent" — expected red state until plan 02-02 creates that module.
...
4. `tests/agent-setup.test.ts`, `tests/intent-form.test.tsx`, `tests/initiate-call-fn.test.ts`, `tests/retell-webhook-fn.test.ts`, `tests/call-detail.test.tsx`, `tests/call-history.test.tsx` — all fail with "Cannot find module" (expected red state)
...
- 6 other test files are in red state (failing with "Cannot find module") — correct arc

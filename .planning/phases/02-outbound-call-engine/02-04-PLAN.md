...
Note: /calls/:id is used for both CallStatus (active calls) and CallDetail (completed calls). Plan 02-05 will replace the CallStatus component on this route with a unified CallDetail page that handles all states.
  ...
  <done>
All 3 tests in intent-form.test.tsx pass. npm run build exits 0 with no TypeScript errors in Dashboard.tsx, CallStatus.tsx, or App.tsx. The /dashboard route renders Dashboard with IntentForm. The /calls/:id route renders CallStatus.
  </done>
</task>
